...
//                                createDialog("Email đã tồn tại").show();
                                createDialog("Email already exists").show();
                                dialog.dismiss();
                            }
                        }
                    });
    ...
    public boolean check() {
        ...
        if (phone.isEmpty()|| name.isEmpty()|| email.isEmpty()|| pass.isEmpty()) {
            createDialog("Fill full the information").show();
            return false;
        }
        else if (!email.matches(String.valueOf(Patterns.EMAIL_ADDRESS))) {
            createDialog("Email invalidate").show();
            return false;
        }
//        else if (!phone.matches("(03|05|07|08|09|01[2689])[0-9]{8}\\b")) {
        else if (phone.matches("")) {
            createDialog("invalid phone number").show();
            return false;
        }
        return true;
    }
